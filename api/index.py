...
def _detect(img_np):
    ...
    for box, sc, ci in zip(bxy, max_s, cls_id):
        ...
        draw.rectangle([x1i, y1i, x2i, y2i], outline="green", width=2)
        draw.rectangle([x1i, y1i - 16, x1i + len(label) * 8 + 4, y1i], fill="green")
        draw.text((x1i + 2, y1i - 15), label, fill="black")
